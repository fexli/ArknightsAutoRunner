...
def _screencap_to_image(cap):
    w, h, pixels = cap
    return np.array(Image.frombytes('RGBA', (w, h), pixels).convert("RGB"))[..., ::-1]
...
def check_adb_alive():
    try:
        sess = ADBClientSession(RootData.get("ADB_SERVER", ("127.0.0.1", 5037)))
...
def ensure_adb_alive():
    ...
    if adbbin is None:
        adb_binaries = ['adb', os.path.join(RootData.get("ADB_SERVER", ("127.0.0.1", 5037)), 'adb')]
...
class ADBConnector:
    def __init__(self, adb_serial=None):
        ...
        self.host_session_factory = lambda: ADBClientSession(RootData.get("ADB_SERVER", ("127.0.0.1", 5037)))
        self.rch = None
    ...
    def _reverse_connection_screencap(self):
        ...
        with future:
            control_sock = self.device_session_factory().exec_stream(
                '(echo -n %s; screencap) | nc %s %d' % (future.cookie.decode(), self.loopback, self.rch.port))
            with control_sock:
