...
MATRIX_FILE_DIR = os.path.dirname(__file__)
Price = dict()
with open(MATRIX_FILE_DIR + '\\price.txt', 'r', encoding='utf8') as f:
...
class MaterialPlanning(object):
    ...
    def get_item_id(self, forceupdate=False):
        if os.path.exists(MATRIX_FILE_DIR + f'/data/items.json') and not forceupdate:
            logger.common("from disk cache load: items.json")
            with open(MATRIX_FILE_DIR + f'/data/items.json', 'r', encoding='utf-8') as f:
                items = json.load(f)
        else:
            logger.common("downloading: items.json")
            items = get_json('items')
            with open(MATRIX_FILE_DIR + f'/data/items.json', 'w', encoding='utf-8') as f:
                json.dump(items, f)
    ...
    def _pre_processing(self, material_probs, forceupdate=False):
        """
        ...
        for server in servers:
            try:
                if os.path.exists(MATRIX_FILE_DIR + f'/data/stages_{server}.json') and not forceupdate:
                    with open(MATRIX_FILE_DIR + f'/data/stages_{server}.json', 'r', encoding='utf-8') as f:
                        logger.common(f"from disk cache load: stages_{server}.json")
                        stages[server] = json.load(f)
                else:
                    logger.common(f"downloading: stages_{server}.json")
                    stages[server] = get_json(f'stages?server={server}')
                    with open(MATRIX_FILE_DIR + f'/data/stages_{server}.json', 'w', encoding='utf-8') as f:
                        json.dump(stages[server], f)
                self.teststage = stages
            except Exception as e:
                logger.error(f'Failed to load server {server}, Error: {e}')
                return -1
        ...
        try:
            self.get_item_id(forceupdate)
        except Exception as e:
            logger.error(f'Failed to load item list, Error: {e}')
            return -1
        ...
        for drop in material_probs['matrix']:
            try:
                ...
                logger.error(f'Failed to parse {drop}. (出现此条请带报错信息联系根派)')
    ...
    def get_plan(self, requirement_dct={}, deposited_dct={},
        ...
        if print_output:
            logger.common('Estimated total cost: %d, gold: %d, exp: %d.' % (res['cost'], res['gold'], res['exp']))
            logger.common('Loot at following stages:')
            for stage in stages:
                display_lst = [k + '(%s) ' % stage['items'][k] for k in stage['items']]
                logger.common('Stage ' + self.stage_code[server][self.stage_name_rv[output_lang][stage['stage']]]
            ...
            for synthesis in syntheses:
                ...
                logger.common(synthesis['target'] + '(%s) <=== ' % synthesis['count']
                              + ', '.join(display_lst))

            logger.common('\nItems Values:')
            for i, group in reversed(list(enumerate(values))):
                display_lst = ['%s:%s' % (item['name'], item['value']) for item in group['items']]
                logger.common('Level %d items: ' % (i + 1))
                logger.common(', '.join(display_lst))
        return res
